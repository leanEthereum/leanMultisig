...
All XMSS public keys are stored in a Merkle tree (the \emph{public-key registry}). Signers are identified by their \emph{index} in this registry (rather than using a bitfield).
...
For the direct source, the program checks each signer's XMSS signature and verifies a Merkle inclusion proof that their public key belongs to the registry.
...
A key subtlety arises from recursion. Every leanVM proof is tied to a specific \emph{bytecode} --- the program that was executed, represented as a multilinear polynomial. In our case, there is a single program (that both contain XMSS verification and recursion algorithms). The leanVM verification algorithm requires evaluating the bytecode polynomial at a random point.
