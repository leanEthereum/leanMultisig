...
Performs a single extension field operation $\text{res} = \text{op}(a, b) \in \Fq$, where $b = \textbf{m}[\nu_B..\nu_B + 5] \in \Fq$, $\text{res} = \textbf{m}[\nu_C..\nu_C + 5] \in \Fq$, and $a$ depends on the mode:

\begin{itemize}
    \item \textbf{BE mode} (\texttt{is\_be} $= 1$): $a = \nu_A \in \Fp$ (direct value, not a pointer), embedded in $\Fq$ as $[\nu_A, 0, 0, 0, 0]$.
    \item \textbf{EE mode} (\texttt{is\_be} $= 0$): $a = \textbf{m}[\nu_A..\nu_A + 5] \in \Fq$ (pointer to extension field element).
\end{itemize}
...
\begin{itemize}
    \item \textbf{ADD}: $\text{res} = a + b$
    \item \textbf{MUL}: $\text{res} = a \cdot b$
    \item \textbf{POLY\_EQ}: $\text{res} = a \cdot b + (1 - a) \cdot (1 - b)$
\end{itemize}

\vspace{2mm}

\texttt{PRECOMPILE\_DATA} is a bitfield encoding both the mode and the operation:
$$\texttt{PRECOMPILE\_DATA} = 2 \cdot \texttt{is\_be} + 4 \cdot \texttt{flag\_add} + 8 \cdot \texttt{flag\_mul} + 16 \cdot \texttt{flag\_poly\_eq}$$
where exactly one of \texttt{flag\_add}, \texttt{flag\_mul}, \texttt{flag\_poly\_eq} is 1. The possible values are:
...
\begin{itemize}
    ...
    \item Extension op precompile $\rightarrow$ PRECOMPILE\_DATA $= 2 \cdot \texttt{is\_be} + 4 \cdot \texttt{flag\_add} + 8 \cdot \texttt{flag\_mul} + 16 \cdot \texttt{flag\_poly\_eq}$
\end{itemize}
...
Each extension field operation occupies a single row. Multiple operations are stacked vertically. Padding rows have all flags set to 0.
...
\toprule
\multicolumn{8}{c|}{Base field columns ($\Fp$)} & \multicolumn{3}{c|}{Extension field columns ($\Fq$)} & \\
\texttt{is\_be} & \texttt{flag} & $\texttt{f}_\texttt{add}$ & $\texttt{f}_\texttt{mul}$ & $\texttt{f}_\texttt{peq}$ & $\text{arg}_A$ & $\text{idx}_B$ & $\text{idx}_R$ & $v_{A,\Fq}$ & $v_B$ & \texttt{res} & \texttt{aux} \\
\midrule
\multicolumn{12}{l}{\textit{MUL, BE ($\nu_A\!=\!42$, $\nu_B\!=\!200$, $\nu_C\!=\!50$): \quad $\texttt{res} = [42,0,0,0,0] \cdot \textbf{m}[200..205]$}} \\[2pt]
1 & 1 & 0 & 1 & 0 & 42 & 200 & 50 & $\textbf{m}[42..47]$ & $\textbf{m}[200..205]$ & $\textbf{m}[50..55]$ & 10 \\
\midrule
\multicolumn{12}{l}{\textit{ADD, EE ($\nu_A\!=\!300$, $\nu_B\!=\!400$, $\nu_C\!=\!500$): \quad $\texttt{res} = \textbf{m}[300..305] + \textbf{m}[400..405]$}} \\[2pt]
0 & 1 & 1 & 0 & 0 & 300 & 400 & 500 & $\textbf{m}[300..305]$ & $\textbf{m}[400..405]$ & $\textbf{m}[500..505]$ & 4 \\
\midrule
\multicolumn{12}{l}{\textit{POLY\_EQ, EE ($\nu_A\!=\!600$, $\nu_B\!=\!700$, $\nu_C\!=\!800$): \quad $\texttt{res} = a \cdot b + (1-a)(1-b)$}} \\[2pt]
0 & 1 & 0 & 0 & 1 & 600 & 700 & 800 & $\textbf{m}[600..605]$ & $\textbf{m}[700..705]$ & $\textbf{m}[800..805]$ & 16 \\
\midrule
\multicolumn{12}{l}{\textit{Padding row}} \\[2pt]
0 & 0 & 0 & 0 & 0 & 0 & 0 & 0 & $\textbf{m}[0..5]$ & $\textbf{m}[0..5]$ & $\textbf{m}[0..5]$ & 0 \\
\bottomrule
\end{tabular}%
}
...
Notes: In BE mode, $\text{arg}_A$ is the direct base field value $\nu_A$ (not a memory address); the lookup $v_{A,\Fq} = \textbf{m}[\text{arg}_A..+5]$ still happens but is not used in the computation. In EE mode, $\text{arg}_A$ is a memory address and $v_{A,\Fq}$ is used. \texttt{aux} is the non-committed bus fingerprinting column, equal to \texttt{PRECOMPILE\_DATA}.

\subsubsection{Columns}

\fbox{8 base field columns + 3 extension field columns = 23 $\Fp$-columns.}
...
\begin{itemize}
    ...
    \item \texttt{flag} $\in \{0,1\}$: bus selector (1 on active rows, 0 on padding)
    \item $\texttt{flag\_add}$ $\in \{0,1\}$: 1 if ADD mode
    \item $\texttt{flag\_mul}$ $\in \{0,1\}$: 1 if MUL mode
    \item $\texttt{flag\_poly\_eq}$ $\in \{0,1\}$: 1 if POLY\_EQ mode
    \item $\text{arg}_A$: direct $\Fp$ value (BE mode) or memory address (EE mode)
    \item $\text{idx}_B$, $\text{idx}_R$: memory addresses for $B$ and the result
\end{itemize}
...
\begin{itemize}
    \item \texttt{aux} $= 2 \cdot \texttt{is\_be} + 4 \cdot \texttt{flag\_add} + 8 \cdot \texttt{flag\_mul} + 16 \cdot \texttt{flag\_poly\_eq}$
\end{itemize}

\subsubsection{Memory lookups}
Every row performs 3 extension field lookups (unconditionally):
\begin{itemize}
    \item $v_{A,\Fq} = \textbf{m}[\text{arg}_A..+5]$ \hfill (in BE mode, the result is not used in the computation)
    \item $v_B = \textbf{m}[\text{idx}_B..+5]$
    \item $\texttt{res} = \textbf{m}[\text{idx}_R..+5]$
\end{itemize}

\subsubsection{Bus interaction}
On rows where $\texttt{flag} = 1$, the table \textsc{Pull}s $(\text{arg}_A, \text{idx}_B, \text{idx}_R, \texttt{aux})$ from the precompile bus. The execution table \textsc{Push}es a matching tuple for each extension op instruction.

Since each of $\texttt{flag\_add}$, $\texttt{flag\_mul}$, $\texttt{flag\_poly\_eq}$ is boolean and exactly one is 1 on active rows, the possible values of \texttt{aux} on active rows are $\{4, 6, 8, 10, 16, 18\}$ (all $\geq 4$), which are disjoint from the Poseidon precompile encoding (1).

\subsubsection{AIR constraints}

\textbf{Value selection:} Define (over $\Fq$, embedding $\text{arg}_A \in \Fp$ as $[\text{arg}_A, 0, 0, 0, 0]$):
$$v_A = \texttt{is\_be} \cdot \text{arg}_A + (1 - \texttt{is\_be}) \cdot v_{A,\Fq} \;\in \Fq$$

\textbf{Constraints:}
\begin{enumerate}
    \item\label{ext:bool-isbe} $\texttt{is\_be} \cdot (1 - \texttt{is\_be}) = 0$ \hfill (\texttt{is\_be} is boolean)
    \item\label{ext:bool-flag} $\texttt{flag} \cdot (1 - \texttt{flag}) = 0$ \hfill (\texttt{flag} is boolean)
    \item\label{ext:bool-fadd} $\texttt{flag\_add} \cdot (1 - \texttt{flag\_add}) = 0$ \hfill (\texttt{flag\_add} is boolean)
    \item\label{ext:bool-fmul} $\texttt{flag\_mul} \cdot (1 - \texttt{flag\_mul}) = 0$ \hfill (\texttt{flag\_mul} is boolean)
    \item\label{ext:bool-fpeq} $\texttt{flag\_poly\_eq} \cdot (1 - \texttt{flag\_poly\_eq}) = 0$ \hfill (\texttt{flag\_poly\_eq} is boolean)
    \item\label{ext:mode-select} $\texttt{flag\_add} + \texttt{flag\_mul} + \texttt{flag\_poly\_eq} = \texttt{flag}$ \hfill (exactly one mode on active rows)
    \item\label{ext:add} $\texttt{flag\_add} \cdot (\texttt{res} - v_A - v_B) = 0$ \hfill (ADD, over $\Fq$)
    \item\label{ext:mul} $\texttt{flag\_mul} \cdot (\texttt{res} - v_A \cdot v_B) = 0$ \hfill (MUL, over $\Fq$)
    \item\label{ext:polyeq} $\texttt{flag\_poly\_eq} \cdot (\texttt{res} - v_A \cdot v_B - (1 - v_A) \cdot (1 - v_B)) = 0$ \hfill (POLY\_EQ, over $\Fq$)
\end{enumerate}

\vspace{3mm}

Since $v_A$ involves \texttt{is\_be} (degree 1) and $v_{A,\Fq}$ (degree 1), the product $v_A \cdot v_B$ is degree 3. Constraints~\ref{ext:mul} and~\ref{ext:polyeq} are therefore degree 4 (flag $\times$ degree-3 expression). There are no transition constraints between rows (no $\text{next}(\cdot)$ references).
...
In case of extension op call, PRECOMPILE\_DATA $= 2 \cdot \texttt{is\_be} + 4 \cdot \texttt{flag\_add} + 8 \cdot \texttt{flag\_mul} + 16 \cdot \texttt{flag\_poly\_eq}$ encodes both the mode and the operation, and $\nu_A$, $\nu_B$, $\nu_C$ represent the first operand (direct value or address), the address of the second operand, and the address of the result.

Each such tuple $\sigma$ (counting multiplicity) must be \textsc{Pulled} by one of the precompile tables. Given that the Poseidon table only pulls tuples with PRECOMPILE\_DATA $= 1$, and the extension op table only pulls tuples with PRECOMPILE\_DATA $\in \{4, 6, 8, 10, 16, 18\}$, the bus is effectively partitioned into two disjoint sub-buses, one for each precompile.
