...
\begin{itemize}
    ...
    \item $[a, b) = \{a, a + 1, \ldots, b-1\}$    \item $eq(x, y) = x y + (1 - x)(1 - y)$
...
\[
\nextmle((x_0, \ldots, x_{n-1}), (y_0, \ldots, y_{n-1})) \;=\; \prod_{j=0}^{n-1} x_j\, y_j \;+\; \sum_{k=0}^{n-1} \Bigg( \prod_{j<k} eq(x_j, y_j) \Bigg)\, (1 - x_k)\, y_k \, \Bigg( \prod_{j>k} x_j\, (1 - y_j) \Bigg) .
\]
...
\prod_{i=0}^{N-1} eq(a_i, b_i) & \text{if } \texttt{op} = \textsc{eq}
\end{cases}
\]
...
\subsection{Stacking multilinear polynomials}
...
\textbf{Sampling of opening points}

For each $i \in [1, n]$, the following points are sampled:
\begin{itemize}
    \item  $(\alpha_i^1, \dots, \alpha_i^{n^\text{normal}_i}) \in (\Fq^{\nu_i})^{n^\text{normal}_i}$, where $n^\text{normal}_i \in \mathbb{N}$ (normal points)
    \item  $(\beta_i^1, \dots, \beta_i^{n^\text{shift}_i}) \in (\Fq^{\nu_i})^{n^\text{shift}_i}$, where $n^\text{shift}_i \in \mathbb{N}$ (shifted points)
\end{itemize}

\textbf{Sending claimed evaluations}

For each $i \in [1, n]$, the prover sends the following:
\begin{itemize}
    \item $\hat{P_i}(\alpha_i^1), \dots, \hat{P_i}(\alpha_i^{n^\text{normal}_i})$ (normal evaluations)
    \item $\overset{\scriptscriptstyle\wedge}{\textsf{shift}(P_i)}(\beta_i^1), \dots, \overset{\scriptscriptstyle\wedge}{\textsf{shift}(P_i)}(\beta_i^{n^\text{shift}_i})$ (shifted evaluations)
\end{itemize}


\textbf{Reducing each claim to a sum over $S$}

Because $\nu_1 \geq \dots \geq \nu_n$, the block of $P_i$ inside $S$ starts at offset $o_i = \sum_{j < i} 2^{\nu_j}$, which is a multiple of $2^{\nu_i}$. Let $\mathsf{sel}_i \in \{0,1\}^{\nu - \nu_i}$ be the big-endian encoding of $o_i / 2^{\nu_i}$ --- the \textbf{boolean selector} of block $i$ (see Figure~\ref{fig:stacking}). Then, as multilinear polynomials,
\[
\hat{P_i}(x) = \hat{S}(\mathsf{sel}_i, x), \qquad \forall x \in \Fq^{\nu_i} .
\]

A \textbf{normal claim} $\hat{P_i}(\alpha) = c$ is therefore an evaluation of the committed polynomial, which we rewrite as a sum over the full hypercube:
\[
c = \hat{S}(\mathsf{sel}_i, \alpha) = \sum_{w \in \{0,1\}^{\nu}} eq\big((\mathsf{sel}_i, \alpha),\, w\big)\; \hat{S}(w) .
\]

A \textbf{shift claim} $\overset{\scriptscriptstyle\wedge}{\textsf{shift}(P_i)}(\beta) = d$ likewise reduces to a sum over $\hat{S}$: the $\nextmle$ identity of \ref{sec:next-mle}, together with $\hat{P_i} = \hat{S}(\mathsf{sel}_i, \cdot)$, gives
\[
d = \sum_{y \in \{0,1\}^{\nu_i}} \nextmle(\beta, y)\; \hat{S}(\mathsf{sel}_i, y) .
\]
Splitting $w \in \{0,1\}^{\nu}$ as $w = (w^{\mathrm{hi}}, w^{\mathrm{lo}})$ with $w^{\mathrm{hi}} \in \{0,1\}^{\nu - \nu_i}$ and $w^{\mathrm{lo}} \in \{0,1\}^{\nu_i}$, this lifts to the full hypercube:
\[
d = \sum_{w \in \{0,1\}^{\nu}} eq(\mathsf{sel}_i, w^{\mathrm{hi}})\cdot \nextmle(\beta, w^{\mathrm{lo}})\; \hat{S}(w) .
\]


TODO

Hence \emph{every} claim --- write $J = \sum_{i=1}^{n}\big(n^\text{normal}_i + n^\text{shift}_i\big)$ for their total number --- takes the common shape
\[
\sum_{w \in \{0,1\}^{\nu}} W_j(w)\; \hat{S}(w) = c_j, \qquad j = 1, \dots, J ,
\]
where the weight $W_j$ is a fixed multilinear that the verifier can efficiently evaluate:
\begin{itemize}
    \item normal claim: $W_j(w) = eq\big((\mathsf{sel}_i, \alpha),\, w\big)$;
    \item shift claim: $W_j(w) = eq(\mathsf{sel}_i, w^{\mathrm{hi}})\cdot \nextmle(\beta, w^{\mathrm{lo}})$
\end{itemize}

\textbf{Batching into a single sumcheck}

The verifier samples a random $\lambda \in \Fq$ and combines the $J$ claims with powers of $\lambda$:
\[
\sum_{j=1}^{J} \lambda^{j} c_j
= \sum_{j=1}^{J} \lambda^{j} \sum_{w \in \{0,1\}^{\nu}} W_j(w)\, \hat{S}(w)
= \sum_{w \in \{0,1\}^{\nu}} W(w)\, \hat{S}(w),
\qquad W := \sum_{j=1}^{J} \lambda^{j} W_j .
\]
If any individual claim is false, this combined identity fails except with probability $\leq J / q$ over $\lambda$. Prover and verifier then run a \textbf{single sumcheck} on
\[
\sum_{w \in \{0,1\}^{\nu}} W(w)\, \hat{S}(w) = C, \qquad C := \sum_{j=1}^{J} \lambda^{j} c_j .
\]
The summand $W \cdot \hat{S}$ is a product of two multilinear polynomials, hence of degree $2$ in each variable: in every one of the $\nu$ rounds the prover sends a degree-$2$ univariate, and the soundness error of the sumcheck is $\leq 2\nu / q$.

\textbf{Concluding}

The sumcheck reduces the statement to its value at the random point $r \in \Fq^{\nu}$ produced over its rounds:
\[
W(r)\cdot \hat{S}(r) = s ,
\]
where $s$ is the final value output by the sumcheck. The verifier evaluates $W(r) = \sum_{j} \lambda^{j} W_j(r)$ on its own --- each $W_j(r)$ is a single $eq$, or a single $eq$ times a single $\nextmle$, all in $O(\nu)$ --- so it only remains to obtain $\hat{S}(r)$. This last claim is discharged by \emph{one} opening of the inner product PCS on the commitment to $S$: taking $x \in \Fq^{2^{\nu}}$ with $\hat{x} = eq(r, \cdot)$, the scheme proves $\sum_{i} S[i]\, x[i] = \hat{S}(r)$. The verifier accepts iff $W(r)\cdot \hat{S}(r) = s$.

In summary, arbitrarily many evaluation and shift claims, on arbitrarily many inner polynomials of arbitrary sizes, collapse into a single $\nu$-round sumcheck followed by a single PCS opening.
...
Example: Consider 3 multilinear polynomials $\hat{P_1}, \hat{P_2}, \hat{P_3}$ with 4, 3, and 2 variables respectively.
...
The trace is split accross 3 tables: execution, poseidon, and extension.
Each table is a 2D matrix, filled by (base) field elements, committed at the beginning of the proof.
