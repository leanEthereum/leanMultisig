...
\fbox{At each cycle, we commit to 19 (base) field elements:}
...
\begin{itemize}
    ...
    \item 11 field elements describing the instruction being executed (see below)
\end{itemize}


\subsubsection{Instruction Encoding}

Each instruction is described by 10 field elements ($\Fp$):

\begin{itemize}
    \item 3 operands: $\text{operand}_A$, $\text{operand}_B$, $\text{operand}_C$
    \item 4 flags ($\in \{0, 1\}$): $\text{flag}_A$, $\text{flag}_B$, $\text{flag}_C$, $\text{flag}_\text{fp}$
    \item 3 opcode flags: MUL ($\in \{0, 1\}$), JUMP ($\in \{0, 1\}$), AUX ($\in \{0, 1, 2, 3\}$) (AUX handles both ADD and DEREF)
...
\begin{itemize}
    ...
    \item $\nu_C = \text{flag}_C \cdot \text{operand}_C + (1 - \text{flag}_C - \text{flag}_\text{fp}) \cdot \text{value}_C + \text{flag}_\text{fp} \cdot \text{fp}$
\end{itemize}

With the associated constraints: 
\begin{itemize}
    \item $(1 - \text{flag}_A) \cdot (\text{address}_A - (\text{fp} + \text{operand}_A)) = 0$
    \item $(1 - \text{flag}_B) \cdot (\text{address}_B - (\text{fp} + \text{operand}_B)) = 0$
    \item $(1 - \text{flag}_C - \text{flag}_\text{fp}) \cdot (\text{address}_C - (\text{fp} + \text{operand}_C)) = 0$
\end{itemize}
...
We define the following virtual columns (non-committed):
...
For DEREF instructions, set AUX $= 2$, $\text{flag}_A = 0$, $\text{flag}_B = 1$ and $\text{flag}_A$ / $\text{flag}_\text{fp}$ according to whether the third operand is an immediate value, a memory read, or the frame pointer. 
