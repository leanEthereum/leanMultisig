...
Every table $T$ has a list of $n^\text{constraints}_T$ AIR (Algebraic Intermediate Representation) constraints. Each AIR constraint $C_k^T$ ($k \in [0, n_\text{constraints}^T)$) is a multivariate polynomial in $n_\text{column}^T + n_\text{shift}^T$ variables. $C_k^T$ is respected by the table's trace if:
$$
\forall r \in [0, 2^{h_T}), \quad C_k(\textsf{col}^T_0(r), \ldots, \textsf{col}^T_{n_\text{column}^T-1}(r), \textsf{shift}(\textsf{col}^T_0)[r], \ldots, \textsf{shift}(\textsf{col}^T_{n_\text{shift}^T-1})[r]) = 0
$$
...
\begin{itemize}
    \item \textbf{AIR transition constraints.} A list of transition constraints (\ref{sec:air-eval}): polynomials in the current and the next row, each required to vanish on every pair of consecutive rows. Their degree is bounded by a fixed value, characteristic of the table.

    \item \textbf{Bus interaction.} A \textit{direction} (\textsc{push} or \textsc{pull}), a boolean \textit{selector} column (possibly virtual), and a \textit{data tuple} of at most $4$ entries, each entry being a column or a constant. On every row where the selector equals $1$, the data tuple is pushed onto (\textsc{push}) or pulled from (\textsc{pull}) the precompile bus; on the other rows the table does not touch the bus. The bus enforces that pushes and pulls match (\ref{buses}).

    \item \textbf{Memory lookups.} A (possibly empty) list of lookups into the read-only memory, each consisting of an \textit{index} column and a group of \textit{value} columns, asserting that the value columns hold the memory contents at consecutive addresses starting from the index (\ref{sec:memory-lookup}).
\end{itemize}

The three tables, together with their maximal heights:
...
Heights are powers of two: each table is padded to the next power of two, with a minimum of $2^{8}$ rows. These bounds are critical for soundness: they ensure that logup multiplicities cannot overflow modulo $p$.
