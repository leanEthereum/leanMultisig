...
\subsubsection{FMA (Fused Multiply-Add with constant)}

A single-instruction form of $\textbf{m}[\textbf{fp} + \beta] = K \cdot \textbf{m}[\textbf{fp} + \delta] + \nu_c$, where $K \in \Fp$ is a compile-time constant. The fp-offset $\delta$ of the multiplicand is carried in the otherwise-unused \texttt{PRECOMPILE\_DATA} slot, and $\alpha$ is repurposed to hold $K$:

$$
\textbf{m}[\textbf{fp} + \beta] = \alpha \cdot \textbf{m}[\textbf{fp} + \texttt{PRECOMPILE\_DATA}] +
\begin{cases}
\gamma \\
\textbf{m}[\textbf{fp} + \gamma] \\
\textbf{fp} + \gamma
...
This collapses the common pattern $v_\text{mul} = K \cdot \textbf{m}[a]$ followed by $\textbf{m}[b] = \textbf{m}[c] + v_\text{mul}$ into one cycle, saving the temporary $v_\text{mul}$ and one instruction.
...
\begin{itemize}
    ...
    \item 3 opcode flags: MUL ($\in \{0, 1\}$), JUMP ($\in \{0, 1\}$), AUX ($\in \{0, 1, 2, 3\}$) (AUX handles ADD, DEREF, and FMA)
    \item PRECOMPILE\_DATA
\end{itemize}

$\text{flag}^{AB}_\textbf{fp}$ is only used by precompile instructions; for ADD, MUL, DEREF, JUMP, and FMA it is always 0.
...
Let $P_0, P_1, P_2, P_3$ be the Lagrange basis polynomials at points $0, 1, 2, 3$ (each of degree $3$). Explicitely:

\begin{align*}
P_0(x) &= -(x-1)(x-2)(x-3) / 6 & P_1(x) &= x(x-2)(x-3) / 2 \\
P_2(x) &= x(x-1)(3-x) / 2 & P_3(x) &= x(x-1)(x-2) / 6
\end{align*}
...
\begin{itemize}
    ...
    \item FMA = $P_3(\text{AUX})$
    \item IS\_PRECOMPILE = $1 - (\text{ADD} + \text{MUL} + \text{DEREF} + \text{JUMP} + \text{FMA})$
\end{itemize}
...
For FMA, set AUX $= 3$, $\text{flag}_A = 1$ (so that $\text{operand}_A$ holds the constant multiplier $K$ and the standard $\text{address}_A$ constraint becomes vacuous), $\text{flag}_B = 0$, and $\text{flag}_C$ / $\text{flag}^{C}_\textbf{fp}$ according to whether the third operand is an immediate value, a memory read, or fp-relative. The fp-offset of the multiplicand is carried by PRECOMPILE\_DATA.

The relation $\textbf{m}[\textbf{fp} + \beta] = \text{operand}_A \cdot \textbf{m}[\textbf{fp} + \texttt{PRECOMPILE\_DATA}] + \nu_C$ is captured by:

\begin{itemize}
    \item $\text{FMA} \cdot (\text{address}_A - (\textbf{fp} + \texttt{PRECOMPILE\_DATA})) = 0$
    \item $\text{FMA} \cdot (\text{value}_B - \text{operand}_A \cdot \text{value}_A - \nu_C) = 0$
\end{itemize}

Degree audit: FMA is a degree-3 selector, $\nu_C$ is degree 2, so $\text{FMA} \cdot (\text{value}_B - \text{operand}_A \cdot \text{value}_A - \nu_C)$ is of degree $3 + 2 = 5$, which fits the AIR's degree-5 budget.
