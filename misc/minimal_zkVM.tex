...
There are two "precompile" instructions: EXTENSION\_OP and POSEIDON2. Each precompile instruction has three (potentially runtime) parameters $\nu_A$, $\nu_B$, $\nu_C$, and a compile-time value \texttt{PRECOMPILE\_DATA}.
...
Performs an accumulated extension field operation over $N$ element pairs ($N \geq 1$). For $i = 0, \ldots, N-1$, define:
$$b_i = \textbf{m}[\nu_B + 5i \,..\, \nu_B + 5i + 5] \in \Fq$$
...
where exactly one of \texttt{flag\_add}, \texttt{flag\_mul}, \texttt{flag\_poly\_eq} is 1, and $N \geq 1$. This is a pure bit-field encoding (each flag occupies a distinct bit position). Since the result is always even and $\geq 32$, it is disjoint from Poseidon ($= 1$) and no-precompile ($= 0$).

\begin{center}
\begin{tabular}{lccc}
\toprule
Operation & EE ($N\!=\!1$) & BE ($N\!=\!1$) & General \\
\midrule
ADD & 36 & 38 & $32N + 4 + 2 \cdot \texttt{is\_be}$ \\
MUL & 40 & 42 & $32N + 8 + 2 \cdot \texttt{is\_be}$ \\
POLY\_EQ & 48 & 50 & $32N + 16 + 2 \cdot \texttt{is\_be}$ \\
\bottomrule
\end{tabular}
\end{center}
...
\toprule
\multicolumn{7}{c|}{Selected base field columns ($\Fp$)} & \multicolumn{4}{c|}{Extension field columns ($\Fq$)} & \\
\texttt{start} & $\texttt{f}_\texttt{mul}$ & \texttt{len} & $\text{arg}_A$ & $\text{idx}_B$ & $\text{idx}_R$ & \texttt{is\_be} & $v_{A,\Fq}$ & $v_B$ & \texttt{comp} & \texttt{res} & \texttt{aux} \\
\midrule
\multicolumn{12}{l}{\textit{MUL, BE, $N\!=\!4$ ($\nu_A\!=\!90$, $\nu_B\!=\!211$, $\nu_C\!=\!74$): \quad $\texttt{res} = \sum_{i=0}^{3} \textbf{m}[90\!+\!i] \cdot \textbf{m}[211\!+\!5i..216\!+\!5i]$}} \\[2pt]
1 & 1 & 4 & 90  & 211 & 74 & 1 & $\textbf{m}[90..95]$  & $\textbf{m}[211..216]$ & $e_0\!+\!e_1\!+\!e_2\!+\!e_3$ & $\textbf{m}[74..79]$ & 138 \\
0 & 1 & 3 & 91  & 216 & 74 & 1 & $\textbf{m}[91..96]$  & $\textbf{m}[216..221]$ & $e_1\!+\!e_2\!+\!e_3$       & $\textbf{m}[74..79]$ & 106 \\
0 & 1 & 2 & 92  & 221 & 74 & 1 & $\textbf{m}[92..97]$  & $\textbf{m}[221..226]$ & $e_2\!+\!e_3$               & $\textbf{m}[74..79]$ & 74 \\
0 & 1 & 1 & 93  & 226 & 74 & 1 & $\textbf{m}[93..98]$  & $\textbf{m}[226..231]$ & $e_3$                       & $\textbf{m}[74..79]$ & 42  \\
\midrule
\multicolumn{12}{l}{\textit{POLY\_EQ, EE, $N\!=\!3$ ($\nu_A\!=\!600$, $\nu_B\!=\!700$, $\nu_C\!=\!800$): \quad $\texttt{res} = \prod_{i=0}^{2} [a_i b_i + (1\!-\!a_i)(1\!-\!b_i)]$}} \\[2pt]
1 & 0 & 3 & 600 & 700 & 800 & 0 & $\textbf{m}[600..605]$ & $\textbf{m}[700..705]$ & $e_0 \cdot e_1 \cdot e_2$ & $\textbf{m}[800..805]$ & 112 \\
0 & 0 & 2 & 605 & 705 & 800 & 0 & $\textbf{m}[605..610]$ & $\textbf{m}[705..710]$ & $e_1 \cdot e_2$           & $\textbf{m}[800..805]$ & 80 \\
0 & 0 & 1 & 610 & 710 & 800 & 0 & $\textbf{m}[610..615]$ & $\textbf{m}[710..715]$ & $e_2$                     & $\textbf{m}[800..805]$ & 48 \\
\midrule
\multicolumn{12}{l}{\textit{Padding row}} \\[2pt]
1 & 0 & 1 & 0   & 0   & 0  & 0 & $\textbf{m}[0..5]$ & $\textbf{m}[0..5]$ & 0 & $\textbf{m}[0..5]$ & 32 \\
\bottomrule
\end{tabular}%
}
...
Notes: $e_i$ denotes the per-element result (virtual, not a committed column). In BE mode, $\text{arg}_A$ is a memory address pointing to a base field element; the lookup $v_{A,\Fq} = \textbf{m}[\text{arg}_A..+5]$ still happens unconditionally but only the first coordinate is used. \texttt{aux} is a non-committed bus fingerprinting column, equal to $2 \cdot \texttt{is\_be} + 4 \cdot \texttt{flag\_add} + 8 \cdot \texttt{flag\_mul} + 16 \cdot \texttt{flag\_poly\_eq} + 32 \cdot \texttt{len}$ on each row. The bus selector is the virtual column $\texttt{activation\_flag} = \texttt{start} \cdot \texttt{active}$ (not shown; equals \texttt{start} on active rows, 0 on padding).
...
\begin{itemize}
    ...
    \item \texttt{start} $\in \{0,1\}$: group boundary (1 on first row of each group \textbf{and} on padding, 0 elsewhere)
    ...
    \item $\text{arg}_A$: memory address (increments by 1 in BE, by 5 in EE within a group)
    ...
    \item $v_{A,\Fp}$: base field value $\textbf{m}[\text{arg}_A]$ (for base field memory lookup)
\end{itemize}

\textbf{Extension field columns ($\Fq$, 4 AIR):}
\begin{itemize}
    \item $v_{A,\Fq}$: extension field value $\textbf{m}[\text{arg}_A .. \text{arg}_A + 5]$ (always looked up; used only in EE mode)
    ...
    \item \texttt{computation}: running accumulation (\textbf{up and down} column); equals \texttt{res} on \texttt{start} rows
\end{itemize}
...
Every row performs 1 base field lookup and 3 extension field lookups (unconditionally):
\begin{itemize}
    \item $v_{A,\Fp} = \textbf{m}[\text{arg}_A]$ \hfill (base field lookup)
    \item $v_{A,\Fq} = \textbf{m}[\text{arg}_A..+5]$ \hfill (in BE mode, only the first coordinate is used)
    \item $v_B = \textbf{m}[\text{idx}_B..+5]$
    \item $\texttt{res} = \textbf{m}[\text{idx}_R..+5]$
\end{itemize}

\subsubsection{Bus interaction}
On rows where $\texttt{activation\_flag} = 1$ (i.e.\ $\texttt{start} = 1$ and $\texttt{active} = 1$), the table \textsc{Pull}s $(\texttt{aux}, \text{arg}_A, \text{idx}_B, \text{idx}_R)$ from the precompile bus. The execution table \textsc{Push}es a matching tuple for each extension op instruction. There is exactly one bus pull per multi-row group.

Since \texttt{aux} $\geq 32$ on all active rows, the encoding is disjoint from the Poseidon precompile (1).

\subsubsection{AIR constraints}

\fbox{17 constraints, degree $\leq$ 6.}

\vspace{2mm}

\textbf{Value selection:} Define (over $\Fq$, embedding $v_{A,\Fp} \in \Fp$ as $[v_{A,\Fp}, 0, 0, 0, 0]$):
$$v_A = \texttt{is\_be} \cdot v_{A,\Fp} + (1 - \texttt{is\_be}) \cdot v_{A,\Fq} \;\in \Fq \qquad \text{(virtual, degree 2)}$$
...
\begin{align*}
    e_\text{add} &= v_A + v_B & \text{(degree 2)} \\
    e_\text{mul} &= v_A \cdot v_B & \text{(degree 3)} \\
    e_\text{peq} &= v_A \cdot v_B + (1 - v_A)(1 - v_B) & \text{(degree 3)}
\end{align*}

Let $\texttt{comp\_tail} = \text{next}(\texttt{comp}) \cdot (1 - \text{next}(\texttt{start}))$ (degree 2; zero when the next row is a new group).

\vspace{2mm}

\textbf{Boolean constraints (5):}
\begin{enumerate}
...
\textbf{Accumulation constraints (3, over $\Fq$):}
\begin{enumerate}
    \setcounter{enumi}{5}
    \item $\texttt{f}_\texttt{add} \cdot (\texttt{comp} - e_\text{add} - \texttt{comp\_tail}) = 0$ \hfill (additive ADD, degree 3)
    \item $\texttt{f}_\texttt{mul} \cdot (\texttt{comp} - e_\text{mul} - \texttt{comp\_tail}) = 0$ \hfill (additive MUL, degree 4)
    \item $\texttt{f}_\texttt{peq} \cdot (\texttt{comp} - e_\text{peq} \cdot (\texttt{comp\_tail} + \text{next}(\texttt{start}))) = 0$ \hfill (multiplicative POLY\_EQ, degree 6)
\end{enumerate}
The backward accumulation works as follows. On the last row of a group ($\text{next}(\texttt{start}) = 1$), $\texttt{comp\_tail} = 0$, so the additive constraints give $\texttt{comp} = e_i$ and the multiplicative gives $\texttt{comp} = e_i \cdot 1 = e_i$. On earlier rows ($\text{next}(\texttt{start}) = 0$), $\texttt{comp} = e_i + \text{next}(\texttt{comp})$ (additive) or $\texttt{comp} = e_i \cdot \text{next}(\texttt{comp})$ (multiplicative). Combined with constraint 9, this gives $\texttt{res} = \sum e_i$ or $\texttt{res} = \prod e_i$.

\vspace{2mm}

\textbf{Result constraint (1, over $\Fq$):}
\begin{enumerate}
    \setcounter{enumi}{8}
    \item $\texttt{start} \cdot (\texttt{comp} - \texttt{res}) = 0$ \hfill (on start rows, computation equals result)
\end{enumerate}

\textbf{Intra-group consistency (7; enforced when $\text{next}(\texttt{start}) = 0$):}
\begin{enumerate}
    \setcounter{enumi}{9}
    ...
    \item $(1 - \text{next}(\texttt{start})) \cdot (\text{next}(\text{arg}_A) - \text{arg}_A - \delta_A) = 0$ \hfill ($\delta_A = \texttt{is\_be} + (1 - \texttt{is\_be}) \cdot 5$)
...
The maximum constraint degree is \textbf{6} (from constraint 8: $\texttt{f\_peq}$ (1) $\times$ $e_\text{peq}$ (3) $\times$ $(\texttt{comp\_tail} + \text{next}(\texttt{start}))$ (2) $= 6$).

\vspace{3mm}

\textbf{Down (next-row) columns:} 8 base field: \texttt{start}, \texttt{is\_be}, \texttt{len}, $\texttt{f}_\texttt{add}$, $\texttt{f}_\texttt{mul}$, $\texttt{f}_\texttt{peq}$, $\text{arg}_A$, $\text{idx}_B$. 1 extension field: \texttt{computation}.
...
Each such tuple $\sigma$ (counting multiplicity) must be \textsc{Pulled} by one of the precompile tables. Given that the Poseidon table only pulls tuples with PRECOMPILE\_DATA $= 1$, and the extension op table only pulls tuples with PRECOMPILE\_DATA $\geq 32$ (always even), the bus is effectively partitioned into two disjoint sub-buses, one for each precompile.
