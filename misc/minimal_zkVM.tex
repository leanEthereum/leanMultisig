...
LeanVM is a verifiable Virtual Machine (also called zkVM for "zero knowledge VM"). It is inspired by Cairo \cite{cairo}: it uses a minimal ISA (Intruction Set Architecture) that operates on a read-only memory where each word is field element.
\begin{itemize}
    ...
    \item It uses the Poseidon \cite{poseidon1} permutation as a hashing primitive
\end{itemize}
