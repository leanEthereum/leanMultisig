...
use lean_vm::{
    ...
    POSEIDON_16_COL_IS_COMPRESSION, POSEIDON_16_COL_RES, POSEIDON_16_COL_RES_HIGH, POSEIDON_16_NULL_HASH_PTR,
    Poseidon16Precompile, ZERO_VEC_PTR, fill_trace_poseidon_16, num_cols_poseidon_16,
};
...
pub fn benchmark_prove_poseidon_16(log_n_rows: usize, tracing: bool) {
    ...
    trace[POSEIDON_16_COL_IS_COMPRESSION] = (0..n_rows).map(|_| F::ONE).collect();
    trace[POSEIDON_16_COL_RES] = (0..n_rows).map(|_| F::from_usize(POSEIDON_16_NULL_HASH_PTR)).collect();
    trace[POSEIDON_16_COL_RES_HIGH] = (0..n_rows).map(|_| F::from_usize(POSEIDON_16_NULL_HASH_PTR)).collect();
