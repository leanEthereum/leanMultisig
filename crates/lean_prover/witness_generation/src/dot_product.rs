use lean_vm::{DIMENSION, EF, F, WitnessDotProduct};
use multilinear_toolkit::prelude::*;

#[derive(Debug)]
pub struct DotProductColumns {
    pub in_base: Vec<Vec<F>>,
    pub in_ext: Vec<Vec<EF>>,
...
pub fn build_dot_product_columns(
    ...
    for dot_product in witness {
        ...
        index_a.extend(
            (0..dot_product.len).map(|i| F::from_usize(dot_product.addr_0 + i * DIMENSION)),
        );
        index_b.extend(
            (0..dot_product.len).map(|i| F::from_usize(dot_product.addr_1 + i * DIMENSION)),
        );
    ...
    DotProductColumns {
        in_base: vec![flag, len, index_a, index_b, index_res],
        in_ext: vec![value_a, value_b, res, computation],
        padding_len,
    }
}
