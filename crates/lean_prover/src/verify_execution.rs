...
pub fn verify_execution(
    ...
    for (table, log_n_rows) in &tables_sorted {
        ...
        verify_data.push(TableVerifyData {
            ...
            eta_power,
            extra_data: ExtraDataForBuses {
                logup_alphas_eq_poly: logup_alphas_eq_poly.to_vec(),
                logup_alphas_eq_poly_packed: logup_alphas_eq_poly
                    .iter()
    ...
    let Evaluation { point: all_challenges, value: final_value } =
        sumcheck_verify(&mut verifier_state, n_max, max_full_degree, initial_sum, None)?;

    let mut check_sum = EF::ZERO;
    for vd in &verify_data {
        let n_cols_total = vd.table.n_columns() + vd.table.n_down_columns();
        let inner_evals = verifier_state.next_extension_scalars_vec(n_cols_total)?;

        macro_rules! eval_constraint {
            ($t:expr) => {{
                <_ as SumcheckComputation<EF>>::eval_extension($t, &inner_evals, &vd.extra_data)
            }};
        }
        let constraint_eval = delegate_to_inner!(&vd.table => eval_constraint);

        let bus_point = from_end(gkr_point, vd.log_n_rows);
        check_sum += back_loaded_table_contribution(
            bus_point,
            &all_challenges.0,
            constraint_eval,
            vd.eta_power,
        ...
        macro_rules! split { ($t:expr) => {{ split_air_inner_evals($t, &inner_evals, &all_challenges.0, vd.log_n_rows) }}; }
        let claim = delegate_to_inner!(&vd.table => split);

        committed_statements.get_mut(&vd.table).unwrap().push(claim);
    }

    if check_sum != final_value {
        return Err(ProofError::InvalidProof);
    }
