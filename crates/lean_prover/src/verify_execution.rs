...
use backend::{Proof, RawProof, VerifierState, VerifierStateSha2};
use lean_vm::*;
use sub_protocols::*;
...
pub fn verify_execution_sha2(
    ...
    let mut verifier_state = VerifierStateSha2::<EF>::new(proof)?;
    verifier_state.observe_scalars(public_input);
    verifier_state.observe_scalars(&poseidon16_compress_pair(&bytecode.hash, &SNARK_DOMAIN_SEP));
    let dims = verifier_state
        .next_base_scalars_vec(3 + N_TABLES)?
    ...
    let parsed_commitment = stacked_pcs_parse_commitment_sha2(
        &whir_config,
        &mut verifier_state,
        log_memory,
        bytecode.log_size(),
