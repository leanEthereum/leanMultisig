...
impl<AB: AirBuilder> Air<AB> for VMAir {
    ...
    fn eval(&self, builder: &mut AB) {
        ...
        let nu_b =
            flag_b.clone() * operand_b.clone() + value_b * (AB::Expr::ONE - flag_b.clone());
        let nu_c = flag_c.clone() * fp.clone() + value_c.clone() * (AB::Expr::ONE - flag_c.clone());

        builder.assert_zero(
            (AB::Expr::ONE - flag_a) * (addr_a - (fp.clone() + operand_a)),
        );
        builder.assert_zero(
            (AB::Expr::ONE - flag_b) * (addr_b - (fp.clone() + operand_b)),
        );
        builder.assert_zero(
        ...
        builder
            .assert_zero(deref.clone() * (addr_c - (value_a + operand_c)));
        builder.assert_zero(deref.clone() * aux.clone() * (value_c.clone() - nu_b.clone()));
        builder.assert_zero(
            deref * (AB::Expr::ONE - aux) * (value_c - fp.clone()),
        );
        ...
        builder.assert_zero(
            jump.clone()
                ...
                * (next_pc - (pc + AB::Expr::ONE)),
        );
        builder.assert_zero(
            jump * (AB::Expr::ONE - nu_a) * (next_fp - fp),
        );
    }
}
