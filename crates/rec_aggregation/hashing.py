...
def batch_hash_slice(num_queries, all_data_to_hash, all_resulting_hashes, num_chuncks):
    if num_chuncks == DIM * 2:
...
def batch_hash_slice_const(num_queries, all_data_to_hash, all_resulting_hashes, num_chuncks: Const):
    for i in range(0, num_queries):
        data = all_data_to_hash[i]
        res = slice_hash(data, num_chuncks)
        all_resulting_hashes[i] = res
    return


@inline
def slice_hash(data, num_chuncks):
    states = Array((num_chuncks - 1) * DIGEST_LEN)
    poseidon16(data, data + DIGEST_LEN, states)
    state_indexes = Array(num_chuncks)
    state_indexes[0] = states
    for j in unroll(1, num_chuncks - 1):
        state_indexes[j] = state_indexes[j - 1] + DIGEST_LEN
        poseidon16(state_indexes[j - 1], data + (j + 1) * DIGEST_LEN, state_indexes[j])
    return state_indexes[num_chuncks - 2]
