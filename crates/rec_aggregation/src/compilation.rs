...
fn compile_main_program(inner_program_log_size: usize, bytecode_zero_eval: F) -> Bytecode {
    ...
    //                   + merkle_chunks_for_slot + tweaks_hash(8) + bytecode_claim + bytecode_hash(8)
    let pub_input_size =
        1 + DIGEST_LEN + 2 + MESSAGE_LEN_FE + N_MERKLE_CHUNKS_FOR_SLOT + DIGEST_LEN + claim_data_size + DIGEST_LEN;
