...
def fs_sample_ef(fs):
    sampled = Array(8)
    poseidon16_compress(fs, ZERO_VEC_PTR, sampled)
    new_fs = Array(9)
    poseidon16_compress(fs, SAMPLING_DOMAIN_SEPARATOR_PTR, new_fs)
...
def fs_sample_data_with_offset(fs, n_chunks: Const, offset):
    ...
    for i in unroll(0, n_chunks):
        ...
        set_to_7_zeros(domain_sep + 1)
        poseidon16_compress(fs, domain_sep, sampled + i * 8)
    return sampled
...
def fs_finalize_sample(fs, total_n_chunks):
    ...
    set_to_7_zeros(domain_sep + 1)
    poseidon16_compress(fs, domain_sep, new_fs)
    new_fs[8] = fs[8]  # same transcript pointer
    return new_fs
