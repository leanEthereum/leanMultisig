...
fn prove_gkr_quotient_step<EF: ExtensionField<PF<EF>>>(
    ...
    let (mut next_point, inner_evals) = match (numerators.by_ref(), denominators.by_ref()) {
        (MleRef::BasePacked(nums), MleRef::ExtensionPacked(dens)) => {
            prove_gkr_quotient_step_mixed(prover_state, nums, dens, claim_point, &extra_data, sum)
        }
        _ => {
...
/// Mixed-type case: manual round 0 with split-eq, fold base→ext, delegate rest.
fn prove_gkr_quotient_step_mixed<EF: ExtensionField<PF<EF>>>(
    prover_state: &mut impl FSProver<EF>,
    nums: &[PFPacking<EF>],
    dens: &[EFPacking<EF>],
    claim_point: &MultilinearPoint<EF>,
    ...
    // Round 0: split-eq compute on mixed-type data
    ...
    // Round 1: FUSED fold (base→ext + ext→ext) + compute in ONE pass.
    ...
    let mmf_2 = eq_eval_0 * eq_eval_1; // fresh SplitEq internally → accumulated_eq

    let group = MleGroupOwned::ExtensionPacked(folded);

    // Delegate remaining rounds (SplitEq built internally, on-the-fly eq)
