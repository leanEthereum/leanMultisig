...
use crate::{
    ALL_PRECOMPILES, CodeAddress, ENDING_PC, HintExecutionContext, N_PRECOMPILES, STARTING_PC, SourceLineNumber,
    TABLE_POSEIDON_16, TABLE_POSEIDON_24, TableTrace,
};
...
fn execute_bytecode_helper(
    ...
    let mut precompile_traces: [TableTrace; N_PRECOMPILES] = array::from_fn(|i| TableTrace::new(&ALL_PRECOMPILES[i]));
    ...
    if precompile_traces[TABLE_POSEIDON_16].base[0].len() + precompile_traces[TABLE_POSEIDON_24].base[0].len() > 0 {
        summary.push_str(&format!(
