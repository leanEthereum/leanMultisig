...
impl<const BUS: bool> TableT for ExecutionTable<BUS> {
    ...
    fn padding_row(&self, padding: &PaddingMemory) -> Vec<F> {
        ...
        padding_row[COL_MEM_ADDRESS_A] = F::from_usize(padding.zero_vec_ptr);
        padding_row[COL_MEM_ADDRESS_B] = F::from_usize(padding.zero_vec_ptr);
        padding_row[COL_MEM_ADDRESS_C] = F::from_usize(padding.zero_vec_ptr);
        padding_row
    }
