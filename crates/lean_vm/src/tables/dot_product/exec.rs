...
pub(super) fn exec_dot_product_be(
...

    {
        ...
        trace.base[DOT_COL_AUX]
            .extend(((1..=size).rev()).map(|x| F::from_usize(DOT_PRODUCT_PRECOMPILE_DATA_BASE + 2 * (1 + 2 * x))));
...
pub(super) fn exec_dot_product_ee(
...

    {
        ...
        trace.base[DOT_COL_LEN].extend(((1..=size).rev()).map(F::from_usize));
        trace.base[DOT_COL_AUX]
            .extend(((1..=size).rev()).map(|x| F::from_usize(DOT_PRODUCT_PRECOMPILE_DATA_BASE + 4 * x)));
