...
impl<const BUS: bool> Air for Poseidon16Precompile<BUS> {
    ...
    fn eval<AB: AirBuilder>(&self, builder: &mut AB, extra_data: &Self::ExtraData) {
        let cols: Poseidon1Cols<AB::F> = {
            let up = builder.up();
            let (prefix, shorts, suffix) = unsafe { up.align_to::<Poseidon1Cols<AB::F>>() };
...
#[repr(C)]
#[derive(Debug)]
pub(super) struct Poseidon1Cols<T> {
    pub flag: T,
    pub index_a: T,
    pub index_b: T,
    pub index_res: T,
...
fn eval<AB: AirBuilder>(builder: &mut AB, local: &Poseidon1Cols<AB::F>) {
    ...
    dense_mat_vec_air(poseidon1_sparse_m_i(), &mut state);
    ...
    for round in 0..PARTIAL_ROUNDS {
        ...
        sparse_mat_air(&mut state, &first_rows[round], &v_vecs[round]);
    }
...
// =========================================================================
// AIR-friendly linear algebra (uses type-dispatched mul_koala_bear)
// =========================================================================

/// Karatsuba-based circulant MDS for full rounds in AIR constraints.
#[inline]
fn mds_karatsuba_air<A: PrimeCharacteristicRing + 'static>(state: &mut [A; WIDTH]) {
    let input = *state;
...
/// Dense 16×16 matrix–vector product (for the one-time m_i transition).
#[inline]
fn dense_mat_vec_air<A: PrimeCharacteristicRing + 'static>(mat: &[[F; 16]; 16], state: &mut [A; WIDTH]) {
    let input = *state;
    for i in 0..WIDTH {
        let mut acc = A::ZERO;
        for j in 0..WIDTH {
...
/// Sparse matrix for partial rounds: new_s0 = dot(first_row, state), state[i] += old_s0 * v[i-1].
#[inline]
fn sparse_mat_air<A: PrimeCharacteristicRing + 'static>(
    state: &mut [A; WIDTH],
    first_row: &[F; WIDTH],
    v: &[F; WIDTH],
) {
...
// =========================================================================
// Type-dispatched field-constant operations
// =========================================================================
...
#[inline(always)]
fn mul_koala_bear<A: PrimeCharacteristicRing + 'static>(a: A, value: F) -> A {
    if TypeId::of::<A>() == TypeId::of::<F>() {
