...
use crate::{
    ...
    tables::{Poseidon1Cols, WIDTH, num_cols_poseidon_16},
};
use backend::*;
...
pub fn fill_trace_poseidon_16(trace: &mut [Vec<F>]) {
    ...
    (0..n / packing_width::<F>()).into_par_iter().for_each(|i| {
        let ptrs: Vec<*mut FPacking<F>> = trace_packed
            ...
            .collect();
        let perm: &mut Poseidon1Cols<&mut FPacking<F>> =
            unsafe { &mut *(ptrs.as_ptr() as *mut Poseidon1Cols<&mut FPacking<F>>) };
    ...
    for i in m..n {
        let ptrs: Vec<*mut F> = trace
            ...
            .collect();
        let perm: &mut Poseidon1Cols<&mut F> = unsafe { &mut *(ptrs.as_ptr() as *mut Poseidon1Cols<&mut F>) };
        generate_trace_rows_for_perm(perm);
    }
}
...
fn generate_trace_rows_for_perm<F: Algebra<KoalaBear> + Copy>(perm: &mut Poseidon1Cols<&mut F>) {
