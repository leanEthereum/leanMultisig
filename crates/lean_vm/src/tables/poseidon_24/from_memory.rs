...
const POSEIDON_24_MEM_COL_FLAG: ColIndex = 0;
const POSEIDON_24_MEM_COL_INDEX_A: ColIndex = 1;
const POSEIDON_24_MEM_COL_INDEX_A_BIS: ColIndex = 2;
const POSEIDON_24_MEM_COL_INDEX_B: ColIndex = 3;
const POSEIDON_24_MEM_COL_INDEX_RES: ColIndex = 4;
const POSEIDON_24_MEM_COL_INPUT_START: ColIndex = 5;
const POSEIDON_24_MEM_COL_OUTPUT_START: ColIndex = POSEIDON_24_MEM_COL_INPUT_START + 24;
// intermediate columns ("commited cubes") are not handled here

#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct Poseidon24MemPrecompile;

impl TableT for Poseidon24MemPrecompile {
    ...
    fn vector_lookups(&self) -> Vec<VectorLookupIntoMemory> {
        vec![
            VectorLookupIntoMemory {
                index: POSEIDON_24_MEM_COL_INDEX_A,
                values: array::from_fn(|i| POSEIDON_24_MEM_COL_INPUT_START + i),
            },
            VectorLookupIntoMemory {
                index: POSEIDON_24_MEM_COL_INDEX_A_BIS,
                values: array::from_fn(|i| POSEIDON_24_MEM_COL_INPUT_START + VECTOR_LEN + i),
            },
            VectorLookupIntoMemory {
                index: POSEIDON_24_MEM_COL_INDEX_B,
                values: array::from_fn(|i| POSEIDON_24_MEM_COL_INPUT_START + 2 * VECTOR_LEN + i),
            },
            VectorLookupIntoMemory {
                index: POSEIDON_24_MEM_COL_INDEX_RES,
                values: array::from_fn(|i| POSEIDON_24_MEM_COL_OUTPUT_START + i),
            },
        ]
    }
    ...
    fn buses(&self) -> Vec<Bus> {
        vec![
            Bus {
                ...
                selector: BusSelector::Column(POSEIDON_24_MEM_COL_FLAG),
                data: vec![
                    POSEIDON_24_MEM_COL_INDEX_A,
                    POSEIDON_24_MEM_COL_INDEX_B,
                    POSEIDON_24_MEM_COL_INDEX_RES,
            ...
            Bus {
                ...
                data: [
                    (POSEIDON_24_MEM_COL_INPUT_START..POSEIDON_24_MEM_COL_INPUT_START + 24).collect::<Vec<ColIndex>>(),
    ...
    fn execute(
        ...
        trace.base[POSEIDON_24_MEM_COL_FLAG].push(F::ONE);
        trace.base[POSEIDON_24_MEM_COL_INDEX_A].push(arg_a);
        trace.base[POSEIDON_24_MEM_COL_INDEX_A_BIS].push(arg_a + F::ONE);
        trace.base[POSEIDON_24_MEM_COL_INDEX_B].push(arg_b);
        trace.base[POSEIDON_24_MEM_COL_INDEX_RES].push(res);
        for (i, value) in input.iter().enumerate() {
            trace.base[POSEIDON_24_MEM_COL_INPUT_START + i].push(*value);
        }
        for (i, value) in output.iter().enumerate() {
            trace.base[POSEIDON_24_MEM_COL_OUTPUT_START + i].push(*value);
        }
...
impl Air for Poseidon24MemPrecompile {
    ...
    fn eval<AB: p3_air::AirBuilder>(&self, builder: &mut AB, extra_data: &Self::ExtraData) {
        ...
        let index_input_a = up[POSEIDON_24_MEM_COL_INDEX_A].clone();
        let index_input_a_bis = up[POSEIDON_24_MEM_COL_INDEX_A_BIS].clone();
        let index_b = up[POSEIDON_24_MEM_COL_INDEX_B].clone();

        let mut core_bus_data = [AB::F::ZERO; 24 + 8];
        core_bus_data[0..24].clone_from_slice(&up[POSEIDON_24_MEM_COL_INPUT_START..][..24]);
        core_bus_data[24..32].clone_from_slice(&up[POSEIDON_24_MEM_COL_OUTPUT_START..][..8]);
