...
pub fn cyclic_subgroup_known_order<F: Field>(generator: F, order: usize) -> impl Iterator<Item = F> + Clone {
...
#[deprecated(note = "use `par_scale_slice_in_place` instead which will replace this method in the future")]
...
pub fn add_scaled_slice_in_place<F: Field>(slice: &mut [F], other: &[F], s: F) {
    ...
    slice_sfx.iter_mut().zip(other_sfx).for_each(|(x, y)| *x += *y * s);
}
...
pub fn reduce_32<SF: PrimeField32, TF: PrimeField>(vals: &[SF]) -> TF {
    ...
    vals.iter()
        .rev()
        .fold(TF::ZERO, |acc, val| acc * base + TF::from_int(val.as_canonical_u32()))
}
