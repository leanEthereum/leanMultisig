...
impl<EF: ExtensionField<PF<EF>>> SplitEq<EF> {
    ...
    /// While eq_lo has > 1 entry, halves eq_lo (split-eq benefit).
    /// Once eq_lo is exhausted, halves eq_hi_packed (flat-table fallback).
    /// When eq_hi_packed reaches 1, further truncation unpacks to remainder mode.
    #[inline]
    ...
    /// Get packed eq value at index i. Uses bit shifts (packed_hi is always a power of 2).
    #[inline(always)]
    ...
    /// Get unpacked eq value at index i.
    #[inline(always)]
