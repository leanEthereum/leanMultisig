...
use crate::poseidon2::{
    ExternalLayer, ExternalLayerConstants, ExternalLayerConstructor, InternalLayer, InternalLayerConstructor, MDSMat4,
    external_initial_permute_state, external_terminal_permute_state,
};
use field::{PrimeCharacteristicRing, mm256_mod_add, mm256_mod_sub};

use crate::{
    FieldParameters, InternalLayerBaseParameters, MontyField31, MontyParameters, PackedMontyField31AVX2,
    PackedMontyParameters, RelativelyPrimePower, apply_func_to_even_odd, halve_avx2, packed_exp_3, packed_exp_5,
    packed_exp_7, signed_add_avx2,
};
...
#[inline(always)]
fn add_rc_and_sbox<PMP: PackedMontyParameters, const D: u64>(val: &mut PackedMontyField31AVX2<PMP>, rc: __m256i) {
    unsafe {
...
pub trait InternalLayerParametersAVX2<PMP: PackedMontyParameters, const WIDTH: usize>: Clone + Sync {
...
where
    FP: FieldParameters,
    ILP: InternalLayerParametersAVX2<FP, 16, ArrayLike = [__m256i; 15]> + InternalLayerBaseParameters<FP, 16>,
{
...
where
    ...
    ILP: InternalLayerParametersAVX2<FP, 24, ArrayLike = [__m256i; 23]> + InternalLayerBaseParameters<FP, 24>,
{
    /// Perform the internal layers of the Poseidon2 permutation on the given state.
