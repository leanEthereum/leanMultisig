...
fn get_cycle_count(path: &str) -> usize {
    ...
    let result = try_execute_bytecode(&bytecode, (&[], &[]), false, &vec![]).unwrap();
    result.pcs.len()
}

#[test]
fn test_constant_if_else_optimization() {
    let path_with_conditions = format!("{}/perf_constant_if_with_conditions.snark", test_data_dir());
    let path_baseline = format!("{}/perf_constant_if_baseline.snark", test_data_dir());
