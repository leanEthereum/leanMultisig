                ...
                let val_offset = match val {
                    ...
                    SimpleExpr::Constant(val_const) => {
                        ...
                        instructions.push(IntermediateInstruction::Computation {
                            ...
                            res: IntermediateValue::MemoryAfterFp {
                                offset: ConstExpression::scalar(temp_offset),
                            },
                        });
                        ConstExpression::scalar(temp_offset)
                    }
                };

                // Allocate 3 auxiliary cells
                let aux1_offset = ConstExpression::scalar(compiler.stack_pos);
                compiler.stack_pos += 1;
                let aux2_offset = ConstExpression::scalar(compiler.stack_pos);
                compiler.stack_pos += 1;
                let aux3_offset = ConstExpression::scalar(compiler.stack_pos);
                compiler.stack_pos += 1;
