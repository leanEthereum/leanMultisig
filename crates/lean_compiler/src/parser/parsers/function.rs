...
use lean_vm::{CUSTOM_HINTS, ExtensionPrecompileAuxArgs, PoseidonMode, Table, TableT};
...
fn is_reserved_function_name(name: &str) -> bool {
    ...
    if PoseidonMode::from_str(name).is_some() {
        return true;
    }
    if ExtensionPrecompileAuxArgs::from_str(name).is_some() {
        return true;
    }
    false
}
