        ...
        Instruction::Precompile(precompile) => {
            let precompile_data = match &precompile.data {
                ...
                PrecompileCompTimeArgs::Memcopy4 { n_reps, stride_in } => {
                    assert!(*n_reps >= 1, "invalid memcopy_4 n_reps={n_reps}");
                    let variant = MEMCOPY4_STRIDES
                        .iter()
                        .position(|&s| s == *stride_in)
                        .expect("invalid memcopy_4 stride_in");
                    EXT_OP_FLAG_MEMCOPY_4 + variant * EXT_OP_FLAG_IS_BE + EXT_OP_LEN_MULTIPLIER * n_reps
                }
            };
