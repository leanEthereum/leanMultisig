/// Clean, structured compiler state following best practices from production compilers.
///
/// This module provides the core compiler structure that manages compilation state
/// in a clean, organized manner similar to LLVM's Module/Function hierarchy.
...
///
/// Manages all compilation state including bytecode generation, memory layout,
/// and variable tracking. Follows the exact structure from the original working compiler.
...
impl Compiler {
    ...
    pub fn compile_program(&mut self, simple_program: SimpleProgram) -> Result<IntermediateBytecode, String> {
        ...
        for function in simple_program.functions.values() {
            ...
            self.bytecode.insert(Label::function(&function.name), instructions);
...
mod tests {
    ...
    fn test_compiler_offset_calculation() {
        ...
        if let ConstExpression::Binary { operation, left, right } = offset {
