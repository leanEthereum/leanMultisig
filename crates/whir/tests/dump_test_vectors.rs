...
/// Replay verifier on a previously-dumped test vector to see if the saved
/// witness still satisfies the grinding check.
#[test]
fn replay_dumped_vector() {
    use fiat_shamir::{ChallengeSampler, FSVerifier};
    use std::fs;
    ...
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join(&target_dir)
        .join("whir_test_vectors")
        .join("small_lir1_nv16.json");
    let text = fs::read_to_string(&path).unwrap();
    let v: serde_json::Value = serde_json::from_str(&text).unwrap();
    let transcript_u32: Vec<u32> = v["proof"]["transcript"]
        .as_array().unwrap().iter()
        .map(|x| x.as_u64().unwrap() as u32)
        .collect();
    let transcript: Vec<F> = transcript_u32.iter().map(|&u| F::from_u32(u)).collect();
    let merkle_paths_json = v["proof"]["merkle_paths"].clone();
    use fiat_shamir::PrunedMerklePaths;
    let merkle_paths: Vec<PrunedMerklePaths<F, F>> = merkle_paths_json
        .as_array().unwrap().iter()
        .map(|bucket| {
            // Manually construct PrunedMerklePaths from JSON.
            let merkle_height = bucket["merkle_height"].as_u64().unwrap() as usize;
            let original_order: Vec<usize> = bucket["original_order"].as_array().unwrap().iter()
                .map(|x| x.as_u64().unwrap() as usize).collect();
            let leaf_data: Vec<Vec<F>> = bucket["leaf_data"].as_array().unwrap().iter()
                .map(|c| c.as_array().unwrap().iter().map(|x| F::from_u32(x.as_u64().unwrap() as u32)).collect())
                .collect();
            let paths: Vec<(usize, Vec<[F; 8]>)> = bucket["paths"].as_array().unwrap().iter()
                .map(|p| {
                    let leaf_index = p["leaf_index"].as_u64().unwrap() as usize;
                    let siblings: Vec<[F; 8]> = p["siblings"].as_array().unwrap().iter()
                        .map(|s| {
                            let arr: Vec<F> = s.as_array().unwrap().iter().map(|x| F::from_u32(x.as_u64().unwrap() as u32)).collect();
                            <[F; 8]>::try_from(arr).unwrap()
                        }).collect();
                    (leaf_index, siblings)
                }).collect();
            let n_trailing_zeros = bucket["n_trailing_zeros"].as_u64().unwrap() as usize;
            PrunedMerklePaths { merkle_height, original_order, leaf_data, paths, n_trailing_zeros }
        }).collect();
    let proof = fiat_shamir::Proof { transcript: transcript.clone(), merkle_paths };

    let poseidon = default_koalabear_poseidon1_16();
    let mut vstate = VerifierState::<EF, _>::new(proof, poseidon).unwrap();

    // Build the verifier WhirConfig and use its commitment_ood_samples count.
    let builder = WhirConfigBuilder {
        security_level: 124,
        max_num_variables_to_send_coeffs: 8,
        pow_bits: 16,
    ...
    let params = WhirConfig::<EF>::new(&builder, 16);
    println!("Rust params.commitment_ood_samples = {}", params.commitment_ood_samples);
    println!("Rust params.starting_folding_pow_bits = {}", params.starting_folding_pow_bits);
    let ood_n = params.commitment_ood_samples;

    // Walk to the first grinding check
    let _root = vstate.next_base_scalars_vec(8).unwrap();
    let _ood_pts: Vec<EF> = vstate.sample_vec(ood_n);
    let _ood_ans = vstate.next_extension_scalars_vec(ood_n).unwrap();
    vstate.duplex();
    let _g: EF = vstate.sample();
    let _poly0 = vstate.next_sumcheck_polynomial(3, EF::ZERO, None).unwrap();
    println!("witness u32 at transcript[23]: {}", transcript_u32[23]);
    let reconstructed = F::from_u32(transcript_u32[23]);
    println!("F::from_u32({}).as_canonical_u32() = {}", transcript_u32[23], reconstructed.as_canonical_u32());
    println!("transcript F repr (Debug):   {:?}", transcript[23]);
    println!("reconstructed F repr (Debug):{:?}", reconstructed);
    match vstate.check_pow_grinding(16) {
        Ok(()) => println!("Rust verifier accepts the saved witness!"),
        Err(e) => println!("Rust verifier ALSO fails: {:?}", e),
    }
}
...
/// Dump Rust's challenger state at every observe/sample for one test vector,
/// so we can diff vs Python.
#[test]
fn dump_state_trace() {
    use fiat_shamir::{ChallengeSampler, FSProver, FSVerifier};
    ...
    let mut prover_state = ProverState::<EF, _>::new(poseidon.clone());
    let polynomial_mle: MleOwned<EF> = MleOwned::Base(polynomial);
    let witness = params.commit(&mut prover_state, &polynomial_mle, num_coeffs);
    params.prove(&mut prover_state, statement.clone(), witness, &polynomial_mle.by_ref());
    let proof = prover_state.into_proof();

    let mut vstate = VerifierState::<EF, _>::new(proof, poseidon.clone()).unwrap();

    // Trace: capture full state (all 16 elements) after each significant op.
    let mut trace: Vec<(String, Vec<u32>)> = Vec::new();
    let push = |t: &mut Vec<(String, Vec<u32>)>, label: &str, st: &VerifierState<EF, _>| {
        let s = FSVerifier::<EF>::state(st);
        let after_state = s.strip_prefix("state ").unwrap_or(&s);
        let elems: Vec<&str> = after_state.split(", ").take(16).collect();
        let full: Vec<u32> = elems
            .iter()
            .map(|e| e.split(" (").next().unwrap().parse::<u32>().unwrap_or(0))
            .collect();
        t.push((label.to_string(), full));
    };

    push(&mut trace, "init", &vstate);
    let _root = vstate.next_base_scalars_vec(8).unwrap();
    push(&mut trace, "after_root", &vstate);
    let _ood_points: Vec<EF> = vstate.sample_vec(params.commitment_ood_samples);
    push(&mut trace, "after_sample_ood_points", &vstate);
    let _ood_answers = vstate
        .next_extension_scalars_vec(params.commitment_ood_samples)
        .unwrap();
    push(&mut trace, "after_read_ood_answers", &vstate);
    vstate.duplex();
    push(&mut trace, "after_duplex", &vstate);
    let _gamma: EF = vstate.sample();
    push(&mut trace, "after_sample_gamma", &vstate);
    let mut cs = EF::ZERO;
    let _poly0 = vstate.next_sumcheck_polynomial(3, cs, None).unwrap();
    push(&mut trace, "after_read_poly0", &vstate);

    #[derive(serde::Serialize)]
    struct Out {
        trace: Vec<(String, Vec<u32>)>,
    }
    let target_dir = std::env::var("CARGO_TARGET_DIR").unwrap_or_else(|_| "target".to_string());
    let out = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join(&target_dir)
        .join("whir_test_vectors")
        .join("state_trace.json");
    fs::write(&out, serde_json::to_string_pretty(&Out { trace: trace.clone() }).unwrap()).unwrap();
...
/// A tiny sanity test for the Python `Challenger`: observe a fixed
/// sequence, then sample a handful of EF challenges, and dump them.
#[test]
fn dump_challenger_vector() {
    use fiat_shamir::{ChallengeSampler, FSProver};

    let poseidon16 = default_koalabear_poseidon1_16();
    let mut prover = ProverState::<EF, _>::new(poseidon16);

    // Observe → sample → re-observe → sample, with an explicit duplex before
    // sampling-after-sample (mirrors how the verifier interleaves).
    let observed: Vec<F> = (1u32..=20).map(F::from_u32).collect();
    prover.observe_scalars(&observed);
    let s1: EF = prover.sample(); // single EF
    prover.duplex();
    let s_vec3: Vec<EF> = prover.sample_vec(3);
    prover.duplex();
    let s_in_range: Vec<usize> = prover.sample_in_range(20, 7);

    #[derive(serde::Serialize)]
    struct Out {
        observed_u32: Vec<u32>,
        sample_ef: [u32; 5],
        sample_ef_vec3: Vec<[u32; 5]>,
        sample_in_range_20_7: Vec<usize>,
    }

    let out = Out {
        observed_u32: observed.iter().map(|&f| f_to_u32(f)).collect(),
        sample_ef: ef_to_u32s(s1),
        sample_ef_vec3: s_vec3.iter().map(|&p| ef_to_u32s(p)).collect(),
        sample_in_range_20_7: s_in_range,
    };
    ...
    let out_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("whir_test_vectors");
    fs::create_dir_all(&out_dir).unwrap();
    let path = out_dir.join("challenger_sanity.json");
    fs::write(&path, serde_json::to_string_pretty(&out).unwrap()).unwrap();
    println!("wrote challenger sanity to {}", path.display());
}
