    ...
    PF<EF>: TwoAdicField,
{
    ...
    pub fn prove2(
        ...
        let mut round_state =
            RoundState2::initialize_first_round_state(self, prover_state, statement, witness, polynomial).unwrap();

        for round in 0..=self.n_rounds() {
            self.round2(round, prover_state, &mut round_state).unwrap();
        }
    ...
    fn round2(
        ...
        round_index: usize,
        prover_state: &mut impl FSProver<EF, Digest = Sha256Digest>,
        round_state: &mut RoundState2<EF>,
    ) -> ProofResult<()> {
        let folded_evaluations = &round_state.sumcheck_prover.evals;
        let num_variables = self.num_variables - self.folding_factor.total_number(round_index);
        ...
        // Base case: final round reached
        if round_index == self.n_rounds() {
            return self.final_round2(round_index, prover_state, round_state);
        }
        ...
        let (ood_challenges, stir_challenges, stir_challenges_indexes) = self.compute_stir_queries2(
            prover_state,
            round_state,
            num_variables,
            round_params,
        ...
        let stir_evaluations = if let Some(data_b) = &round_state.commitment_merkle_prover_data_b {
            let answers_a = open_merkle_tree_at_challenges2(
                &round_state.merkle_prover_data,
                prover_state,
                &stir_challenges_indexes,
            );
            let answers_b = open_merkle_tree_at_challenges2(data_b, prover_state, &stir_challenges_indexes);
            let mut stir_evaluations = Vec::new();
            for (answer_a, answer_b) in answers_a.iter().zip(&answers_b) {
                let vars_a = answer_a.by_ref().n_vars();
                let vars_b = answer_b.by_ref().n_vars();
            ...
            open_merkle_tree_at_challenges2(&round_state.merkle_prover_data, prover_state, &stir_challenges_indexes)
                .iter()
                .map(|answer| answer.evaluate(&folding_randomness))
                .collect()
        };
    ...
    #[allow(clippy::type_complexity)]
    fn compute_stir_queries2(
        &self,
        prover_state: &mut impl FSProver<EF>,
        round_state: &RoundState2<EF>,
        num_variables: usize,
        round_params: &RoundConfig<EF>,
        ood_points: &[EF],
        round_index: usize,
...
fn open_merkle_tree_at_challenges2<EF: ExtensionField<PF<EF>>>(
    merkle_tree: &MerkleData2<EF>,
    prover_state: &mut impl FSProver<EF, Digest = Sha256Digest>,
    stir_challenges_indexes: &[usize],
) -> Vec<MleOwned<EF>> {
    let mut answers = Vec::new();
...
#[derive(Debug)]
pub(crate) struct RoundState2<EF>
where
    EF: ExtensionField<PF<EF>>,
{
    domain_size: usize,
    next_domain_gen: PF<EF>,
    sumcheck_prover: SumcheckSingle<EF>,
    commitment_merkle_prover_data_b: Option<MerkleData2<EF>>,
    merkle_prover_data: MerkleData2<EF>,
    randomness_vec: Vec<EF>,
}
