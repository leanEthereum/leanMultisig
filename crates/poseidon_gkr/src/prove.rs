use crate::{
    EF, F, FullRoundComputation, GKRPoseidonResult, PartialRoundComputation, apply_matrix, build_poseidon_inv_matrix,
    poseidon_round_constants,
};
use backend::*;
use tracing::{info_span, instrument};

/// Prove the GKR statement that the columns at `gkr_col_start..` form a valid
/// Poseidon1-WIDTH permutation trace, starting from input columns `input_col_start..input_col_start + WIDTH`.
///
/// `output_point` is the multilinear evaluation point on which the output-layer's evaluation claim is made.
/// `perm_out_0_7` is the verifier-derivable first WIDTH/2 evaluations of the output layer at `output_point`
/// (derived from `compressed_output_eval - input_eval`). The remaining WIDTH/2 are evaluated by the prover and sent.
#[instrument(skip_all)]
pub fn prove_poseidon_gkr<const WIDTH: usize>(
    ...
    let n_poseidons = trace[input_col_start].len();
    ...
    let initial_start = gkr_col_start;
    ...
    // sanity: output_layer is the LAST GKR layer.
